...
from app.models import Card, Review, User, SessionStats
...
def log_review(
    ...
    stats = db.query(SessionStats).filter(
        ...
        SessionStats.category == card.language
    ).first()
    
    if stats:
        stats.minutes_spent += max(1, review_data.time_spent_seconds // 60)
        stats.card_count += 1
    else:
        stats = SessionStats(
            user_id=user.id,
            ...
            minutes_spent=max(1, review_data.time_spent_seconds // 60),
            card_count=1
        )
        db.add(stats)
